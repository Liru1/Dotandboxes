...
public class Board {
     ...
     * @param size
     */
    public Board(Stage stage, boolean playAI, boolean playPeople, int size) {
        ...
        if (playAI && !playPeople) {
            setCircleEventHandlingAgainstAI();
        } else if (!playAI && playPeople) {
    ...
    public void makeComputerMove() {
        ...
        while (temp == 0) {
            ...
            for (Line line : lines) {
                for (int i = 0; i < boxes.size(); i++) {
                    if (!boxes.get(i).checkIfLineMatchAndFilled(line)) {
                        if (!line.isVisible()) {//line is not in this box and not visible
                            legalMoves.add(line);
                        }
                    }
                }
            ...
            for (Line l : legalMoves) {
                templines.add(l);
            }
            Random rand = new Random();
            int randomMove = rand.nextInt(legalMoves.size());
            for (int k = 0; k < boxes.size(); k++) {
                boxes.get(k).findAndSetLineColor(templines.get(randomMove), playerOneTurn);
            }
            templines.get(randomMove).setVisible(true);
            if (checkForFilledBoxes()) {
                if (checkForWin()) {
                    return;
                }
